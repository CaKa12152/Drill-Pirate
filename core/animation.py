...
def motion_window_for_dot(
    drill_set: DrillSet,
    dot_id: str,
    default_end_count: float | None = None,
) -> tuple[float, float]:
    set_start = float(drill_set.start_count)
    authored_end = float(drill_set.end_count)
    set_end = max(authored_end, float(default_end_count if default_end_count is not None else authored_end))
    timing = drill_set.move_timings.get(dot_id, {})
    move_start = float(timing.get("start", set_start))
    move_end = float(timing.get("end", set_end))
    if (
        timing
        and abs(move_start - set_start) < 0.0001
        and abs(move_end - authored_end) < 0.0001
    ):
        move_end = set_end
...
def interpolate_dot_facings(project: DrillProject, set_index: int, count: float) -> dict[str, float]:
    ...
    transition_end = transition_end_count(project, current_index)
    ...
    for dot in project.dots:
        ...
        motion_start_count, motion_end_count = motion_window_for_dot(current, dot.id, transition_end)
...
def interpolate_props(project: DrillProject, set_index: int, count: float) -> dict[str, dict[str, float]]:
    ...
    movement_counts = max(1.0, transition_end_count(project, current_index) - motion_start_count)
