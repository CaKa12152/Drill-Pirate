...
def transition_end_count(project: DrillProject, set_index: int) -> float:
    """Return the temporal boundary after the set's final inclusive count."""
    if not project.sets:
        return 1.0
    index = max(0, min(set_index, len(project.sets) - 1))
    drill_set = project.sets[index]
    if index + 1 < len(project.sets):
        next_start = float(project.sets[index + 1].start_count)
        if next_start > float(drill_set.start_count):
            return next_start
...
def audio_ms_to_count(project: DrillProject, milliseconds: float) -> float:
    ...
    if not anchors and not project.timing_events:
        return legacy_audio_ms_to_count(project, milliseconds)
...
def legacy_audio_ms_to_count(project: DrillProject, milliseconds: float) -> float:
    if not project.sets:
        return 1.0
    start = show_start_count(project)
    end = show_end_count(project)
    target_ms = max(0.0, float(milliseconds))
    elapsed_ms = 0.0
    boundaries = sorted(
        {
            start,
            ...
            *[
                ...
                for drill_set in project.sets
                if start < float(drill_set.start_count) < end
            ],
        }
    )
    if len(boundaries) < 2:
    ...
    for index in range(len(boundaries) - 1):
        ...
        segment_midpoint = (segment_start + segment_end) / 2.0
        tempo = project.active_tempo(set_index_for_count(project, segment_midpoint))
        milliseconds_per_count = 60000.0 / max(1.0, tempo)
        segment_ms = (segment_end - segment_start) * milliseconds_per_count
        if target_ms <= elapsed_ms + segment_ms or index == len(boundaries) - 2:
            count = segment_start + (target_ms - elapsed_ms) / milliseconds_per_count
            return max(start, min(count, end))
