...
from drill_writer.core.curvilinear import (
    CurvilinearFormPlan,
    initialize_curvilinear_nodes,
    plan_curvilinear_form,
    plan_curvilinear_transition,
)
...
from drill_writer.core.playback import (
    AudioClockInterpolator,
    FrameScheduler,
    PlaybackFrameCache,
    PlaybackQuality,
)
...
"""
    ...
    def tool_label(self, tool: EditorTool) -> str:
        labels = {
            ...
            EditorTool.CURVILINEAR: "Curvilinear",
            EditorTool.ARC: "Arc",
...
TOOL_HINTS: dict[EditorTool, str] = {
    ...
    EditorTool.CURVILINEAR: "Build editable flowing ranks on a master spline, then make marchers follow the curve between sets.",
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, project_dir: Path) -> None:
        ...
        self.curvilinear_nodes: list[dict[str, tuple[float, float]]] = []
        self.curvilinear_plan: CurvilinearFormPlan | None = None
        self.curvilinear_rank_dot_ids: list[list[str]] = []
        ...
        self.smoothed_audio_clock = AudioClockInterpolator()
        ...
        self.player.playbackStateChanged.connect(self.audio_playback_state_changed)
        self.player.mediaStatusChanged.connect(self.audio_media_status_changed)
    ...
    def build_menus(self) -> None:
        ...
        tool_shortcuts = (
            ...
            ("Curvilinear Tool", EditorTool.CURVILINEAR, "Ctrl+Alt+Shift+U"),
        )
    ...
    def tool_command_id(self, tool: EditorTool) -> str:
        return {
            ...
            EditorTool.CURVILINEAR: "curvilinear_tool",
    ...
    def recover_saved_audio_output_device(self) -> None:
        self.check_audio_output_health()
    ...
    def apply_audio_output_device(
        ...
        *,
        ...
        if self.player.source().isValid():
            self.reset_smoothed_audio_clock(position)
    ...
    def audio_playback_state_changed(self, state: QMediaPlayer.PlaybackState) -> None:
        if state != QMediaPlayer.PlaybackState.StoppedState or not self.play_timer.isActive():
            return
        if self.audio_recovery_policy.recovering:
            return
        self.stop_playback_transport()
        if self.player.mediaStatus() != QMediaPlayer.MediaStatus.EndOfMedia:
            self.statusBar().showMessage(
                "Audio playback stopped unexpectedly. Press Play to resume.",
                4500,
            )

    def audio_media_status_changed(self, status: QMediaPlayer.MediaStatus) -> None:
        if status != QMediaPlayer.MediaStatus.EndOfMedia:
            return
        self.stop_playback_transport()
        self.statusBar().showMessage(
            "Reached the end of the audio. Press Play to restart the show.",
            3500,
        )
    ...
    def recover_audio_output(self) -> None:
        ...
        if not success:
            ...
            return
        if self.player.source().isValid():
            self.reset_smoothed_audio_clock(self.audio_recovery_position_ms)
    ...
    def build_tools_panel(self) -> QWidget:
        ...
        self.curve_advanced_handles = QCheckBox("Show Advanced Curve Handles")
        self.curve_advanced_handles.setToolTip(
            "Show two additional on-curve controls for asymmetric bends. Leave this off for simple pull-to-curve editing."
        )
        self.curve_advanced_handles.toggled.connect(self.update_formation_preview)
        ...
        curve_note = QLabel(
            "Drag the purple Pull Curve handle directly away from the selected line for one smooth bend. "
            "Enable advanced handles only when you need an asymmetric curve."
        )
        curve_note.setWordWrap(True)
        curve_form.addRow("Default Bend", self.curve_bend)
        curve_form.addRow(self.curve_advanced_handles)
        ...
        self.curvilinear_tool_group = QGroupBox("Curvilinear Form")
        curvilinear_form = QFormLayout(self.curvilinear_tool_group)
        self.curvilinear_node_count = QSpinBox()
        self.curvilinear_node_count.setRange(2, 16)
        self.curvilinear_node_count.setValue(6)
        self.curvilinear_node_count.setToolTip("Number of red spline nodes shown directly on the field.")
        self.curvilinear_rank_count = QSpinBox()
        self.curvilinear_rank_count.setRange(1, 12)
        self.curvilinear_rank_count.setValue(1)
        self.curvilinear_rank_count.setToolTip("Build parallel curved ranks from one shared master spline.")
        self.curvilinear_rank_interval = QDoubleSpinBox()
        self.curvilinear_rank_interval.setRange(0.25, 12.0)
        self.curvilinear_rank_interval.setSingleStep(0.25)
        self.curvilinear_rank_interval.setValue(2.0)
        self.curvilinear_rank_interval.setSuffix(" yd")
        self.curvilinear_closed = QCheckBox("Closed Loop")
        self.curvilinear_closed.setToolTip("Connect the last node to the first for enclosed curvilinear forms.")
        self.curvilinear_follow_path = QCheckBox("Flow Along Curves Between Sets")
        self.curvilinear_follow_path.setChecked(True)
        self.curvilinear_follow_path.setToolTip(
            "Create smooth shared travel paths instead of sending each marcher straight to the destination dot."
        )
        self.curvilinear_flow_strength = QDoubleSpinBox()
        self.curvilinear_flow_strength.setRange(0.0, 150.0)
        self.curvilinear_flow_strength.setSingleStep(5.0)
        self.curvilinear_flow_strength.setValue(80.0)
        self.curvilinear_flow_strength.setSuffix(" %")
        self.curvilinear_flow_strength.setToolTip("How strongly travel bends with the source and destination rank tangents.")
        self.curvilinear_face_direction = QCheckBox("Face Along Curvilinear Line")
        self.curvilinear_face_direction.setToolTip(
            "Keep every marcher tangent to the Curvilinear rank at the beginning and destination, while following "
            "the movement route between them. The preview updates immediately when the spline changes."
        )
        reset_curvilinear_button = QPushButton("Reset Spline From Selection")
        reset_curvilinear_button.clicked.connect(self.reset_curvilinear_nodes)
        curvilinear_note = QLabel(
            "Drag red nodes to reshape the line. Drag cyan handles to control entry and exit curvature. "
            "Purple lines are parallel ranks; gold paths are the exact transition preview."
        )
        curvilinear_note.setWordWrap(True)
        self.curvilinear_node_count.valueChanged.connect(self.reset_curvilinear_nodes)
        self.curvilinear_rank_count.valueChanged.connect(self.reset_curvilinear_rank_assignment)
        self.curvilinear_rank_interval.valueChanged.connect(self.update_formation_preview)
        self.curvilinear_closed.toggled.connect(self.reset_curvilinear_nodes)
        self.curvilinear_follow_path.toggled.connect(self.update_formation_preview)
        self.curvilinear_flow_strength.valueChanged.connect(self.update_formation_preview)
        self.curvilinear_face_direction.toggled.connect(self.update_formation_preview)
        curvilinear_form.addRow("Spline Nodes", self.curvilinear_node_count)
        curvilinear_form.addRow("Parallel Ranks", self.curvilinear_rank_count)
        curvilinear_form.addRow("Rank Interval", self.curvilinear_rank_interval)
        curvilinear_form.addRow(self.curvilinear_closed)
        curvilinear_form.addRow(self.curvilinear_follow_path)
        curvilinear_form.addRow("Travel Curvature", self.curvilinear_flow_strength)
        curvilinear_form.addRow(self.curvilinear_face_direction)
        curvilinear_form.addRow(reset_curvilinear_button)
        curvilinear_form.addRow("", curvilinear_note)
    ...
    def seek_audio_position(self, position_ms: int) -> None:
        ...
        self.sync_timeline()
        if self.player.source().isValid():
            self.reset_smoothed_audio_clock(position_ms)
    ...
    def set_tool(self, tool: EditorTool) -> None:
        ...
        elif tool == EditorTool.CURVILINEAR and positions:
            self.initialize_curvilinear_tool(positions)
        if previous_tool == EditorTool.CURVILINEAR and tool != EditorTool.CURVILINEAR:
            self.field.set_facings(self.facings_for_set())
        self.update_formation_preview()
    ...
    def tool_value_text(self, kind: str, x: float, y: float, modifiers: int) -> str:
        ...
        if kind.startswith("curvilinear_node:"):
            return f"Curvilinear node  X {x:.2f}  Y {y:.2f}{suffix}"
        if kind.startswith("curvilinear_tangent:"):
            return f"Curvilinear tangent  X {x:.2f}  Y {y:.2f}{suffix}"
    ...
    def update_tool_edit_visibility(self) -> None:
        ...
        self.curvilinear_tool_group.setVisible(tool == EditorTool.CURVILINEAR)
    ...
    def curve_on_form_handles(self, offset_x: float, offset_y: float) -> dict[str, tuple[float, float]]:
        ...
        result = {
            "curve_start": start,
            "curve_pull": cubic_bezier_point(start, control_1, control_2, end, 0.5),
            "curve_end": end,
        }
        if self.curve_advanced_handles.isChecked():
            result["curve_on_1"] = cubic_bezier_point(start, control_1, control_2, end, 1 / 3)
            result["curve_on_2"] = cubic_bezier_point(start, control_1, control_2, end, 2 / 3)
        return result

    def set_curve_pull_point(self, point: tuple[float, float]) -> None:
        start = self.curve_handles["curve_start"]
        end = self.curve_handles["curve_end"]
        midpoint = ((start[0] + end[0]) / 2.0, (start[1] + end[1]) / 2.0)
        offset = (
            (point[0] - midpoint[0]) * 4.0 / 3.0,
            (point[1] - midpoint[1]) * 4.0 / 3.0,
        )
        self.curve_handles["curve_control_1"] = (
            start[0] + (end[0] - start[0]) / 3.0 + offset[0],
            start[1] + (end[1] - start[1]) / 3.0 + offset[1],
        )
        self.curve_handles["curve_control_2"] = (
            start[0] + (end[0] - start[0]) * 2.0 / 3.0 + offset[0],
            start[1] + (end[1] - start[1]) * 2.0 / 3.0 + offset[1],
        )
        delta_x = end[0] - start[0]
        delta_y = end[1] - start[1]
        length = max(0.001, (delta_x * delta_x + delta_y * delta_y) ** 0.5)
        signed_bend = offset[0] * (-delta_y / length) + offset[1] * (delta_x / length)
        previous_blocked = self.curve_bend.blockSignals(True)
        self.curve_bend.setValue(signed_bend)
        self.curve_bend.blockSignals(previous_blocked)
    ...
    def reset_curvilinear_nodes(self, *_args) -> None:
        _ids, positions = self.selected_positions()
        self.initialize_curvilinear_tool(positions)
        self.update_formation_preview()

    def initialize_curvilinear_tool(self, positions: list[tuple[float, float]]) -> None:
        self.curvilinear_plan = None
        dot_ids, _selected_positions = self.selected_positions()
        self.curvilinear_rank_count.setMaximum(max(1, len(positions)))
        existing_assignment = [list(row) for row in self.curvilinear_rank_dot_ids]
        if self.valid_curvilinear_rank_assignment(existing_assignment, dot_ids):
            self.curvilinear_rank_dot_ids = existing_assignment
        else:
            self.curvilinear_rank_dot_ids = self.saved_curvilinear_rank_assignment(dot_ids)
        if len(positions) < 2:
            self.curvilinear_nodes = []
            return
        self.curvilinear_nodes = initialize_curvilinear_nodes(
            positions,
            self.curvilinear_node_count.value(),
            closed=self.curvilinear_closed.isChecked(),
        )

    def valid_curvilinear_rank_assignment(
        self,
        assignment: list[list[str]],
        dot_ids: list[str],
    ) -> bool:
        if len(assignment) != self.curvilinear_rank_count.value():
            return False
        flattened = [dot_id for row in assignment for dot_id in row]
        return len(flattened) == len(dot_ids) and set(flattened) == set(dot_ids)

    def saved_curvilinear_rank_assignment(self, dot_ids: list[str]) -> list[list[str]]:
        signature = ",".join(sorted(dot_ids))
        if not signature:
            return []
        bucket = self.workflow_bucket("formation_edit_descriptors")
        for set_index in (self.set_index, self.set_index - 1):
            if set_index < 0:
                continue
            settings = bucket.get(f"{set_index}:{signature}")
            if not isinstance(settings, dict):
                continue
            raw_assignment = settings.get("curvilinear_rank_dot_ids")
            if not isinstance(raw_assignment, list):
                continue
            assignment = [
                [str(dot_id) for dot_id in row]
                for row in raw_assignment
                if isinstance(row, list)
            ]
            flattened = [dot_id for row in assignment for dot_id in row]
            if len(flattened) != len(dot_ids) or set(flattened) != set(dot_ids):
                continue
            previous_blocked = self.curvilinear_rank_count.blockSignals(True)
            self.curvilinear_rank_count.setValue(len(assignment))
            self.curvilinear_rank_count.blockSignals(previous_blocked)
            if self.valid_curvilinear_rank_assignment(assignment, dot_ids):
                return assignment
        return []

    def reset_curvilinear_rank_assignment(self, *_args) -> None:
        self.curvilinear_rank_dot_ids = []
        self.curvilinear_plan = None
        self.update_formation_preview()
    ...
    def current_tool_settings(self) -> dict[str, Any]:
        return {
            ...
            "curve_advanced_handles": self.curve_advanced_handles.isChecked(),
            ...
            "curvilinear_node_count": self.curvilinear_node_count.value(),
            "curvilinear_rank_count": self.curvilinear_rank_count.value(),
            "curvilinear_rank_interval": self.curvilinear_rank_interval.value(),
            "curvilinear_closed": self.curvilinear_closed.isChecked(),
            "curvilinear_follow_path": self.curvilinear_follow_path.isChecked(),
            "curvilinear_flow_strength": self.curvilinear_flow_strength.value(),
            "curvilinear_face_direction": self.curvilinear_face_direction.isChecked(),
            ...
            "curvilinear_nodes": [
                {name: list(point) for name, point in node.items()}
                for node in self.curvilinear_nodes
            ],
            "curvilinear_rank_dot_ids": [list(row) for row in self.curvilinear_rank_dot_ids],
    ...
    def apply_tool_settings(self, settings: dict[str, Any]) -> None:
        ...
        for attr in (
            ...
            "curvilinear_node_count",
            "curvilinear_rank_count",
            "curvilinear_rank_interval",
            "curvilinear_flow_strength",
            "scatter_radius",
            "scatter_spacing",
            "shape_radius",
            "shape_width",
        ...
        if "curve_advanced_handles" in settings:
            self.curve_advanced_handles.setChecked(bool(settings["curve_advanced_handles"]))
        ...
        if "curvilinear_closed" in settings:
            self.curvilinear_closed.setChecked(bool(settings["curvilinear_closed"]))
        if "curvilinear_follow_path" in settings:
            self.curvilinear_follow_path.setChecked(bool(settings["curvilinear_follow_path"]))
        if "curvilinear_face_direction" in settings:
            self.curvilinear_face_direction.setChecked(bool(settings["curvilinear_face_direction"]))
        ...
        raw_curvilinear_nodes = settings.get("curvilinear_nodes")
        if isinstance(raw_curvilinear_nodes, list):
            parsed_nodes: list[dict[str, tuple[float, float]]] = []
            for raw_node in raw_curvilinear_nodes:
                if not isinstance(raw_node, dict):
                    continue
                node: dict[str, tuple[float, float]] = {}
                for name in ("point", "in", "out"):
                    point = raw_node.get(name)
                    if isinstance(point, (list, tuple)) and len(point) >= 2:
                        node[name] = (float(point[0]), float(point[1]))
                if "point" in node:
                    parsed_nodes.append(node)
            if parsed_nodes:
                self.curvilinear_nodes = parsed_nodes
        raw_rank_ids = settings.get("curvilinear_rank_dot_ids")
        if isinstance(raw_rank_ids, list):
            self.curvilinear_rank_dot_ids = [
                [str(dot_id) for dot_id in row]
                for row in raw_rank_ids
                if isinstance(row, list)
            ]
        ...
        for dot_id, (x, y) in positions.items():
            ...
            if sync_unchanged_set_one_endpoints and set_one is not None:
                ...
                if self.positions_match(endpoint, old_position):
                    ...
                    endpoint_synced = True
            if not self.positions_match(old_position, new_position):
                dot.x, dot.y = new_position
                changed += 1
            if facings is not None and set_one is not None and dot_id in facings:
                facing = float(facings[dot_id]) % 360.0
                start_count = float(set_one.start_count)
                existing_start = set_one.count_facings.get(dot_id, {}).get(start_count)
                if existing_start is None or abs(float(existing_start) - facing) > 0.001:
                    set_one.count_facings.setdefault(dot_id, {})[start_count] = facing
                    changed += 1
                if endpoint_synced:
                    existing_end = set_one.dot_facings.get(dot_id)
                    if existing_end is None or abs(float(existing_end) - facing) > 0.001:
                        set_one.dot_facings[dot_id] = facing
                        changed += 1
        if not changed:
            self.set_count(self.current_count, seek_audio=False)
            return
    ...
    def formation_targets(self, tool: EditorTool) -> dict[str, tuple[float, float]]:
        ...
        elif tool == EditorTool.CURVILINEAR:
            if len(self.curvilinear_nodes) != int(self.curvilinear_node_count.value()):
                self.initialize_curvilinear_tool(positions)
            nodes = [
                {
                    name: (point[0] + offset_x, point[1] + offset_y)
                    for name, point in node.items()
                }
                for node in self.curvilinear_nodes
            ]
            try:
                self.curvilinear_plan = plan_curvilinear_form(
                    ids,
                    {
                        dot_id: self.current_transition_start_positions().get(dot_id, position)
                        for dot_id, position in zip(ids, positions)
                    },
                    nodes,
                    rank_count=self.curvilinear_rank_count.value(),
                    rank_interval=self.curvilinear_rank_interval.value(),
                    closed=self.curvilinear_closed.isChecked(),
                    preferred_rank_dot_ids=self.curvilinear_rank_dot_ids,
                )
            except ValueError:
                self.curvilinear_plan = None
                return {}
            self.curvilinear_rank_dot_ids = [list(row) for row in self.curvilinear_plan.rank_dot_ids]
            targets = self.snap_form_mapping_to_grid(dict(self.curvilinear_plan.targets))
            self.curvilinear_plan.targets = dict(targets)
            return targets
    ...
    def formation_handles(self, tool: EditorTool) -> dict[str, tuple[float, float]]:
        ...
        offset_x, offset_y = self.preview_center_offset
        handles: dict[str, tuple[float, float]] = {}
        if tool not in (EditorTool.MIRROR, EditorTool.SHAPE_LINE, EditorTool.CURVE):
    ...
    def update_formation_preview(self) -> None:
        ...
        if self.field.active_tool == EditorTool.CURVILINEAR:
            plan = self.curvilinear_plan
            if plan is None:
                self.field.clear_preview()
                return
            transition = None
            editing_opening = self.editing_set_one_opening()
            transition_available = (
                (self.curvilinear_follow_path.isChecked() and not editing_opening)
                or self.curvilinear_face_direction.isChecked()
            )
            if transition_available:
                transition = plan_curvilinear_transition(
                    plan.rank_dot_ids,
                    starts,
                    targets,
                    flow_strength=(
                        self.curvilinear_flow_strength.value() / 100.0
                        if self.curvilinear_follow_path.isChecked()
                        else 0.0
                    ),
                    face_direction=self.curvilinear_face_direction.isChecked(),
                    closed=self.curvilinear_closed.isChecked(),
                )
                travel_paths = (
                    transition.paths
                    if self.curvilinear_follow_path.isChecked() and not editing_opening
                    else {
                        dot_id: [starts[dot_id], target]
            ...
            target_facings: dict[str, float] = {}
            if self.curvilinear_face_direction.isChecked() and transition is not None:
                existing_facings = self.facings_for_set()
                if editing_opening:
                    self.field.set_facings(existing_facings)
                else:
                    source_facings = {
                        dot_id: facings.get(0.0, existing_facings.get(dot_id, 0.0))
                        for dot_id, facings in transition.count_facings.items()
                    }
                    self.field.set_facings(source_facings)
                target_facings = dict(transition.end_facings)
            else:
                self.field.set_facings(self.facings_for_set())
            offset_x, offset_y = self.preview_center_offset
            display_nodes = [
            ...
            self.field.show_curvilinear_preview(
                plan.master_path,
                plan.rank_paths,
                travel_paths,
                targets,
    ...
    def preview_handle_moved(
        ...
        if self.move_motion_ribbon_handle(kind, x, y, modifiers, commit):
            return
        if self.move_curvilinear_handle(kind, x, y, modifiers):
            return
        shift = bool(modifiers & int(Qt.KeyboardModifier.ShiftModifier.value))
        alt = bool(modifiers & int(Qt.KeyboardModifier.AltModifier.value))
        ...
        elif kind == "curve_pull":
            if not self.curve_handles:
                self.initialize_curve_tool(positions)
            self.set_curve_pull_point((x - offset_x, y - offset_y))
    ...
    def move_curvilinear_handle(
        ...
        if not kind.startswith("curvilinear_"):
            return False
        offset_x, offset_y = self.preview_center_offset
        base_position = (x - offset_x, y - offset_y)
        alt = bool(modifiers & int(Qt.KeyboardModifier.AltModifier.value))
        if kind.startswith("curvilinear_node:"):
            try:
                ...
                return True
            if not 0 <= index < len(self.curvilinear_nodes):
                return True
            node = self.curvilinear_nodes[index]
            old_point = node["point"]
            delta = (base_position[0] - old_point[0], base_position[1] - old_point[1])
            node["point"] = base_position
            for control_name in ("in", "out"):
                control = node.get(control_name)
                if control is not None:
                    node[control_name] = (control[0] + delta[0], control[1] + delta[1])
            if alt and len(self.curvilinear_nodes) > 2:
                opposite_index = len(self.curvilinear_nodes) - 1 - index
                if opposite_index != index:
                    _ids, positions = self.selected_positions()
                    center_x, center_y = self.positions_center(positions)
                    opposite = self.curvilinear_nodes[opposite_index]
                    mirrored = (center_x * 2 - base_position[0], center_y * 2 - base_position[1])
                    opposite_old = opposite["point"]
                    opposite_delta = (mirrored[0] - opposite_old[0], mirrored[1] - opposite_old[1])
                    opposite["point"] = mirrored
                    for control_name in ("in", "out"):
                        control = opposite.get(control_name)
                        if control is not None:
                            opposite[control_name] = (
                                control[0] + opposite_delta[0],
                                control[1] + opposite_delta[1],
                            )
        elif kind.startswith("curvilinear_tangent:"):
            parts = kind.split(":")
            if len(parts) != 3:
                return True
            try:
            ...
            if control_name not in {"in", "out"} or not 0 <= index < len(self.curvilinear_nodes):
                return True
            node = self.curvilinear_nodes[index]
            node[control_name] = base_position
            if alt:
                opposite_name = "out" if control_name == "in" else "in"
                point = node["point"]
                node[opposite_name] = (
                    point[0] * 2 - base_position[0],
                    point[1] * 2 - base_position[1],
                )
        self.curvilinear_plan = None
        self.update_formation_preview()
        return True
    ...
    def apply_current_preview(self) -> None:
        if self.active_plugin_form_tool_id:
            ...
            return
        if self.field.active_tool == EditorTool.CURVILINEAR:
            self.apply_curvilinear_formation()
            return
        self.apply_formation(self.field.active_tool)

    def apply_curvilinear_formation(self) -> None:
        repeat_settings = deepcopy(self.current_tool_settings())
        targets = self.editable_positions(self.formation_targets(EditorTool.CURVILINEAR))
        plan = self.curvilinear_plan
        if not targets or plan is None:
            return
        if self.editing_set_one_opening():
            opening_facings: dict[str, float] | None = None
            if self.curvilinear_face_direction.isChecked():
                opening_transition = plan_curvilinear_transition(
                    plan.rank_dot_ids,
                    targets,
                    targets,
                    flow_strength=0.0,
                ...
                opening_facings = opening_transition.end_facings
            self.remember_formation_edit_descriptor(targets, repeat_settings)
            self.apply_opening_positions(
                targets,
                sync_unchanged_set_one_endpoints=True,
                label="Apply Curvilinear Opening Form",
                facings=opening_facings,
            )
            self.field.clear_preview()
            self.preview_center_offset = (0.0, 0.0)
            self.set_tool(EditorTool.SELECT)
            self.refresh_selected_paths()
            self.remember_repeat_action(
                {"type": "formation", "settings": repeat_settings, "label": "Apply Curvilinear Form"}
            )
            return
        ...
        before_index = self.set_index
        before_count = self.current_count
        target_indices = self.selected_set_indices_for_edit(list(targets))
        for target_index in target_indices:
            drill_set = self.project.sets[target_index]
            starts_source = (
                self.project.sets[target_index - 1].dot_positions
                if target_index > 0
                else {dot.id: (dot.x, dot.y) for dot in self.project.dots}
            )
            starts = {
                dot_id: starts_source.get(dot_id, drill_set.dot_positions.get(dot_id, target))
                for dot_id, target in targets.items()
            }
            for dot_id, target in targets.items():
                drill_set.dot_positions[dot_id] = target
                drill_set.path_anchors.pop(dot_id, None)
                drill_set.path_controls.pop(dot_id, None)
                drill_set.count_positions.pop(dot_id, None)
                drill_set.count_facings.pop(dot_id, None)
            if not (
                self.curvilinear_follow_path.isChecked()
                or self.curvilinear_face_direction.isChecked()
            ):
                continue
            transition = plan_curvilinear_transition(
            ...
            motion_start = float(drill_set.start_count)
            motion_end = transition_end_count(self.project, target_index)
            for dot_id in targets:
                anchors = transition.path_anchors.get(dot_id, [])
                controls = transition.path_controls.get(dot_id, [])
                if self.curvilinear_follow_path.isChecked() and anchors:
                    drill_set.path_anchors[dot_id] = list(anchors)
                    drill_set.path_controls[dot_id] = deepcopy(controls)
                if dot_id in transition.count_facings:
                    span = max(0.0001, motion_end - motion_start)
                    drill_set.count_facings[dot_id] = {
                        round(motion_start + progress * span, 6): facing
                        for progress, facing in transition.count_facings[dot_id].items()
                        if 0.0 <= progress < 1.0
                    }
                    drill_set.dot_facings[dot_id] = transition.end_facings[dot_id]

        self.remember_formation_edit_descriptor(targets, repeat_settings)
        self.set_count(self.current_count, seek_audio=False)
        self.push_set_snapshot(before_sets, before_index, before_count, "Apply Curvilinear Form")
        self.field.clear_preview()
        self.preview_center_offset = (0.0, 0.0)
        self.set_tool(EditorTool.SELECT)
        self.select_dot_ids(self.ordered_dot_ids(list(targets)))
        self.refresh_selected_paths()
        self.remember_repeat_action(
            {"type": "formation", "settings": repeat_settings, "label": "Apply Curvilinear Form"}
        )
        path_text = " with spline-following travel" if self.curvilinear_follow_path.isChecked() else ""
        self.statusBar().showMessage(
            f"Applied {len(plan.rank_dot_ids)} curvilinear rank(s) to {len(targets)} marchers{path_text}",
            3800,
        )
    ...
    def context_action(self, name: str) -> None:
        ...
        mapping = {
            ...
            "Preview Curvilinear": EditorTool.CURVILINEAR,
    ...
    def set_count(
        ...
        if seek_audio and self.player.source().isValid():
            ...
            self.reset_smoothed_audio_clock(target_position)
    ...
    def finish_playback_frame(self, render_started: float) -> None:
        ...
        if quality is not None:
            self.sync_playback_timer_quality(quality)
    ...
    def play(self) -> None:
        if self.play_timer.isActive():
            if (
                ...
                return
            self.stop_playback_transport(refresh_paths=False)
        self.audio_recovery_resume_requested = False
        if self.playback_is_at_end():
            self.restart_playback_from_beginning()
        self.playback_scheduler.set_adaptive(
            self.settings.value("performance/adaptive_playback", True, type=bool)
        )
        self.playback_scheduler.reset(perf_counter() * 1000.0)
        self.sync_playback_timer_quality(self.playback_scheduler.quality)
        self.playback_frame_cache.clear()
        self.playback_auxiliary_frame = 0
        self.field.set_playback_quality("full", len(self.project.dots))
        if self.player.source().isValid():
            start_audio_ms = self.audio_position_for_count(self.set_index, self.current_count)
            self.reset_smoothed_audio_clock(start_audio_ms)
            self.player.setPosition(start_audio_ms)
    ...
    def stop_playback_transport(self, *, refresh_paths: bool = True) -> None:
        self.play_timer.stop()
        self.field.set_playback_quality("full", len(self.project.dots))
    ...
    def playback_is_at_end(self) -> bool:
        if not self.project.sets:
            return False
        last_set_index = len(self.project.sets) - 1
        _show_start, show_end = playback_bounds_for_set(self.project, last_set_index)
        at_final_count = self.set_index == last_set_index and self.current_count >= show_end - 0.001
        media_at_end = (
            self.player.source().isValid()
            and self.player.mediaStatus() == QMediaPlayer.MediaStatus.EndOfMedia
        )
        return at_final_count or media_at_end

    def restart_playback_from_beginning(self) -> None:
        self.set_index = 0
        show_start, _show_end = playback_bounds_for_set(self.project, 0)
        self.current_count = show_start
        self.populate_sets()
        self.sync_timeline()
        self.set_count(show_start, seek_audio=False)
        if self.player.source().isValid():
            start_audio_ms = self.audio_position_for_count(0, show_start)
            if self.player.mediaStatus() == QMediaPlayer.MediaStatus.EndOfMedia:
                self.player.stop()
            self.reset_smoothed_audio_clock(start_audio_ms)
            self.player.setPosition(start_audio_ms)
    ...
    def tick_playback(self) -> None:
        ...
        if self.player.source().isValid():
            audio_position = self.smoothed_audio_clock.sample(
            ...
            if self.loop_current_set.isChecked() and audio_position >= max(loop_start_ms, loop_end_ms - 2):
                ...
                self.reset_smoothed_audio_clock(loop_start_ms)
    ...
    def reset_smoothed_audio_clock(self, position_ms: int) -> None:
        self.last_playback_audio_ms = max(0, int(position_ms))
        self.smoothed_audio_clock.reset(
            self.last_playback_audio_ms,
            perf_counter() * 1000.0,
            self.current_playback_rate(),
        )

    def sync_playback_timer_quality(self, quality: PlaybackQuality) -> None:
        interval_ms = max(8, int(1000.0 / quality.render_fps))
        if self.play_timer.interval() != interval_ms:
    ...
    def refresh_selected_paths(self) -> None:
        ...
        self.field.show_paths(paths, anchors, controls, show_handles=len(selected) <= 3)
