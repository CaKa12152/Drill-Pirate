...
class DotItem(QGraphicsEllipseItem):
    ...
    def set_facing_degrees(self, facing_degrees: float) -> None:
        normalized = float(facing_degrees) % 360.0
        if abs(normalized - self.facing_degrees) < 0.001:
            return
        self.facing_degrees = normalized
        self.update()
...
class FieldView(QGraphicsView):
    ...
    def __init__(self) -> None:
        ...
        self._drag_press_view_position: QPointF | None = None
    ...
    def set_playback_quality(self, quality: str, performer_count: int = 0) -> None:
        ...
        if normalized == "full":
            ...
            suppress_labels = performer_count >= 240
        else:
            hints = QPainter.RenderHint(0)
            suppress_labels = performer_count >= 120
    ...
    def set_positions(self, positions: dict[str, tuple[float, float]]) -> None:
        for dot_id, position in positions.items():
            item = self.dot_items.get(dot_id)
            if item:
                target = self.field_to_scene(*position)
                current = item.pos()
                if abs(current.x() - target.x()) > 0.0001 or abs(current.y() - target.y()) > 0.0001:
                    item.setPos(target)
        if self._active_transform_handle is None and not self.transform_gizmo_suspended:
            self.update_transform_gizmo()
    ...
    def show_curvilinear_preview(
        self,
        master_path: list[tuple[float, float]],
        rank_paths: list[list[tuple[float, float]]],
        travel_paths: dict[str, list[tuple[float, float]]],
        targets: dict[str, tuple[float, float]],
        nodes: list[dict[str, tuple[float, float]]],
        target_facings: dict[str, float] | None = None,
    ) -> None:
        self.clear_preview()
        for rank_path in rank_paths:
            if len(rank_path) < 2:
                continue
            rank_item = QGraphicsPathItem(self.make_painter_path(rank_path))
            rank_item.setPen(QPen(QColor("#a855f7"), 0.22 * self.scale_factor, Qt.PenStyle.DashLine))
            rank_item.setAcceptedMouseButtons(Qt.MouseButton.NoButton)
            rank_item.setZValue(11)
            self.scene.addItem(rank_item)
            self.preview_items.append(rank_item)
        if len(master_path) > 1:
            master_item = ShapeLineItem(self.make_painter_path(master_path), self.scale_factor, "curvilinear")
            master_item.setPen(QPen(QColor("#e53935"), 0.52 * self.scale_factor))
            master_item.setToolTip("Curvilinear master spline — drag red nodes or cyan tangent handles")
            self.scene.addItem(master_item)
            self.preview_items.append(master_item)
        travel_pen = QPen(QColor(247, 209, 84, 175), 0.11 * self.scale_factor, Qt.PenStyle.DashLine)
        combined_travel = QPainterPath()
        for path in travel_paths.values():
            if len(path) < 2:
                continue
            first = self.field_to_scene(*path[0])
            combined_travel.moveTo(first)
            for point in path[1:]:
                combined_travel.lineTo(self.field_to_scene(*point))
        if not combined_travel.isEmpty():
            travel_item = QGraphicsPathItem(combined_travel)
            travel_item.setPen(travel_pen)
            travel_item.setAcceptedMouseButtons(Qt.MouseButton.NoButton)
            travel_item.setZValue(12)
            self.scene.addItem(travel_item)
            self.preview_items.append(travel_item)
        radius = FIELD_DOT_RADIUS_YARDS * self.scale_factor
        preview_facings = target_facings or {}
        for dot_id, target in targets.items():
            target_item = DotSymbolPreviewItem(
                radius,
                QColor(247, 209, 84, 145),
                ...
                QColor("#fff2a6"),
                FIELD_DOT_OUTLINE_YARDS * self.scale_factor,
                rotation_degrees=preview_facings.get(dot_id, 0.0),
            )
            target_item.setPos(self.field_to_scene(*target))
            self.scene.addItem(target_item)
            self.preview_items.append(target_item)
        for index, node in enumerate(nodes):
            ...
            if point is None:
                continue
            node_handle = PreviewHandleItem(f"curvilinear_node:{index}", self.scale_factor)
            node_radius = 0.32 * self.scale_factor
            node_handle.setRect(-node_radius, -node_radius, node_radius * 2, node_radius * 2)
            node_handle.setBrush(QColor("#e53935"))
            node_handle.setPen(QPen(QColor("#ffffff"), 0.15 * self.scale_factor))
            node_handle.setToolTip("Drag spline node; hold Alt to mirror the opposite node")
            node_handle.setPos(self.field_to_scene(*point))
            self.scene.addItem(node_handle)
            self.add_preview_handle_label(node_handle, f"P{index + 1}")
            self.preview_items.append(node_handle)
            for control_name in ("in", "out"):
                ...
                if control is None:
                    continue
                point_scene = self.field_to_scene(*point)
                control_scene = self.field_to_scene(*control)
                tangent_line = QGraphicsLineItem()
                tangent_line.setLine(point_scene.x(), point_scene.y(), control_scene.x(), control_scene.y())
                tangent_line.setPen(QPen(QColor("#66d9ef"), 0.1 * self.scale_factor, Qt.PenStyle.DotLine))
                tangent_line.setAcceptedMouseButtons(Qt.MouseButton.NoButton)
                tangent_line.setZValue(13)
                self.scene.addItem(tangent_line)
                self.preview_items.append(tangent_line)
                tangent = PreviewHandleItem(f"curvilinear_tangent:{index}:{control_name}", self.scale_factor)
                tangent_radius = 0.22 * self.scale_factor
                tangent.setRect(-tangent_radius, -tangent_radius, tangent_radius * 2, tangent_radius * 2)
                tangent.setBrush(QColor("#66d9ef"))
                tangent.setPen(QPen(QColor("#0b1d24"), 0.13 * self.scale_factor))
                tangent.setToolTip("Drag tangent; hold Alt to mirror the opposite tangent")
                tangent.setPos(control_scene)
                self.scene.addItem(tangent)
                self.preview_items.append(tangent)
    ...
    def update_labels(self, enabled: bool) -> None:
        ...
        suppress_labels = self.playback_quality == "performance" and len(self.dot_items) >= 120
        suppress_labels = suppress_labels or (
            self.playback_quality == "balanced" and len(self.dot_items) >= 240
        )
        for item in self.dot_items.values():
    ...
    def rebuild_dots(self) -> None:
        ...
        for dot in self.project.dots:
            ...
            suppress_labels = self.playback_quality == "performance" and len(self.project.dots) >= 120
            suppress_labels = suppress_labels or (
                self.playback_quality == "balanced" and len(self.project.dots) >= 240
            )
            item.label.setVisible(self.show_labels and not suppress_labels)
    ...
    def cancel_active_interaction(self) -> None:
        ...
        self._drag_press_view_position = None
        self._drag_started = False
        self.clear_lasso()
        self.clear_snap_guides()
        self.hide_tool_value()
    ...
    def mousePressEvent(self, event) -> None:  # type: ignore[override]
        ...
        self._drag_start_positions = {}
        self._drag_start_prop_states = {}
        self._drag_press_view_position = QPointF(event.position())
        self._drag_started = False
        if (
        ...
        if not self._drag_start_positions and not self._drag_start_prop_states:
            self._drag_press_view_position = None
    ...
    def mouseMoveEvent(self, event) -> None:  # type: ignore[override]
        ...
        if self.active_tool == EditorTool.SELECT and (
            self._drag_start_positions or self._drag_start_prop_states
        ):
            self._drag_started = self._drag_started or self.drag_threshold_reached(event.position())
        if (
            self.active_tool == EditorTool.SELECT
        ...
        if self.active_tool == EditorTool.SELECT and self._drag_started and self._drag_start_positions:
    ...
    def mouseReleaseEvent(self, event) -> None:  # type: ignore[override]
        ...
        if self.active_tool != EditorTool.SELECT:
            ...
            self.clear_snap_guides()
            return
        if self._drag_start_positions or self._drag_start_prop_states:
            self._drag_started = self._drag_started or self.drag_threshold_reached(event.position())
        if not self._drag_started and (self._drag_start_positions or self._drag_start_prop_states):
            self.set_positions(self._drag_start_positions)
            for prop_id, state in self._drag_start_prop_states.items():
                item = self.prop_items.get(prop_id)
                if item:
    ...
    def drag_threshold_reached(self, position: QPointF) -> bool:
        if self._drag_press_view_position is None:
            return False
        delta = position - self._drag_press_view_position
        threshold = max(2, QApplication.startDragDistance())
